...
export default function AdminUsersListPage() {
  ...
  if (error) {
    return (
      <div className="p-6">
        <p className="text-sm text-green-primary">{error}</p>
      </div>
    )
  }
      ...
      <div className="bg-light-base dark:bg-dark-card border border-light-border dark:border-dark-border rounded-xl divide-y divide-light-border dark:divide-dark-border">
        ...
        {users.map(user => {
          ...
          return (
            ...
            >
              {/* Avatar */}
              <div className="w-10 h-10 bg-green-primary/10 flex items-center justify-center flex-shrink-0 text-green-primary font-bold text-sm">
                {initials}
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {user.is_suspended && (
                    <span className="text-[10px] font-bold px-1.5 py-0.5 bg-green-primary/10 text-green-primary uppercase tracking-wider">
                      Suspended
                    </span>
                  )}
                  {userPending > 0 && (
              ...
              {/* Arrow */}
              <ChevronRight size={16} className="text-slate-400 group-hover:text-green-primary transition-colors flex-shrink-0" />
            </Link>
          )
        })}
      </div>
