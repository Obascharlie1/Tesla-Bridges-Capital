    ...
    <div>
      ...
      <button
        ...
        className={`w-full border-2 border-dashed transition-colors p-6 text-center ${
          file ? 'border-emerald-400 bg-emerald-50 dark:bg-emerald-900/10' : 'border-light-border dark:border-dark-border hover:border-green-primary hover:bg-green-primary/5'
        }`}
      >
        {file ? (
    ...
    return (
      <div>
        ...
        <div className="p-6 max-w-lg mx-auto mt-12 text-center">
          ...
          <Link href="/dashboard" className="inline-flex items-center gap-2 px-6 py-3 bg-green-primary text-white font-bold text-sm hover:bg-green-dim transition-colors">
            Back to Dashboard
          </Link>
        </div>
      </div>
    ...
    <div>
      ...
      <div className="p-4 sm:p-6 max-w-xl mx-auto space-y-6">
        ...
        <div className="flex gap-3 p-4 bg-light-surface dark:bg-dark-surface border border-light-border dark:border-dark-border">
          <Shield size={18} className="text-green-primary flex-shrink-0 mt-0.5" />
          <div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {docTypes.map(dt => (
                <label
                  ...
                  className={`flex items-start gap-3 p-4 border cursor-pointer transition-colors ${
                    docType === dt.value ? 'border-green-primary bg-green-primary/5' : 'border-light-border dark:border-dark-border hover:border-green-primary/50'
                  }`}
                >
                  <input
                    type="radio"
                    ...
                    className="mt-0.5 accent-green-primary flex-shrink-0"
                  />
                  <div>
            ...
            <ul className="space-y-1">
              {[
                ...
                <li key={req} className="flex items-start gap-2 text-xs text-slate-500 dark:text-slate-400">
                  <span className="w-1 h-1 bg-green-primary flex-shrink-0 mt-1.5" />
                  {req}
                </li>
              ))}
            </ul>
          ...
          <button
            ...
            className="w-full bg-green-primary hover:bg-green-dim disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold py-3.5 text-sm flex items-center justify-center gap-2 transition-colors"
          >
            {loading
