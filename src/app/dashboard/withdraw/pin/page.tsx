      ...
      <div className="p-6 max-w-4xl mx-auto">
        ...
        {phase === 'success' ? (
          ...
          <div className="bg-light-base dark:bg-dark-card border border-light-border dark:border-dark-border rounded-xl p-8 max-w-sm mx-auto">
            ...
            {/* Error */}
            {errorMsg && (
              <p className="text-center text-xs text-green-primary mb-4 font-medium">{errorMsg}</p>
            )}
              ...
              {padKeys.map((key, i) => {
                if (key === 'backspace') {
                  return (
                    <button key={i} type="button" onClick={handleBackspace} disabled={phase === 'submitting'}
                      className="w-16 h-16 flex items-center justify-center border border-light-border dark:border-dark-border text-slate-500 dark:text-slate-400 hover:border-green-primary hover:text-green-primary transition-colors disabled:opacity-40">
                      <Delete size={18} />
                    </button>
                  )
                }
                if (key === 'clear') {
                  return (
                    <button key={i} type="button" onClick={handleClear} disabled={phase === 'submitting'}
                      className="w-16 h-16 flex items-center justify-center border border-light-border dark:border-dark-border text-slate-500 dark:text-slate-400 hover:border-green-primary hover:text-green-primary transition-colors text-xs font-bold uppercase tracking-wider disabled:opacity-40">
                      CLR
                    </button>
                  )
                }
                return (
                  <button key={i} type="button" onClick={() => handleDigit(key)} disabled={phase === 'submitting'}
                    className="w-16 h-16 flex items-center justify-center border border-light-border dark:border-dark-border text-dark-base dark:text-white hover:border-green-primary hover:text-green-primary hover:bg-green-primary/5 transition-colors text-lg font-bold disabled:opacity-40">
                    {key}
                  </button>
                )
              })}
