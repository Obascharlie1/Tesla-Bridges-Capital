      ...
      <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-4 sm:space-y-6">
        ...
        <div className="grid grid-cols-3 sm:flex sm:flex-wrap gap-3">
          {[
            ...
            { href: '/dashboard/plans',        icon: TrendingUp,      label: 'Plans'    },
            { href: '/dashboard/transactions', icon: History,         label: 'History', wide: true },
          ].map(({ href, icon: Icon, label, wide }) => (
            <Link
              key={href}
              href={href}
              className={`flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-1.5 sm:gap-2 px-4 py-3 sm:py-2.5 border border-light-border dark:border-dark-border text-xs sm:text-sm font-semibold text-dark-base dark:text-white hover:border-red-primary hover:text-red-primary transition-colors ${wide ? 'col-span-3 sm:col-span-1 flex-row' : ''}`}
            >
