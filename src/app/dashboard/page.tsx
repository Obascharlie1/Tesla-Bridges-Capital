...
function statusClasses(status: string) {
  ...
  if (status === 'Rejected')  return 'bg-green-primary/10 text-green-primary'
      ...
      <div className="p-4 sm:p-6 max-w-6xl mx-auto space-y-4 sm:space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          ...
          <div className="bg-light-base dark:bg-dark-card border border-light-border dark:border-dark-border rounded-xl p-5 sm:p-6">
            ...
            <p className="text-xl sm:text-2xl font-bold text-green-primary">+${(profile?.profit ?? 0).toLocaleString()}</p>
          </div>
          ...
          <div className="col-span-2 lg:col-span-4 bg-light-base dark:bg-dark-card border border-light-border dark:border-dark-border rounded-xl p-5 sm:p-6 flex items-center gap-4">
            ...
            <Link href="/dashboard/plans" className="ml-auto flex items-center gap-1.5 text-xs font-semibold text-green-primary hover:text-green-dim transition-colors whitespace-nowrap">
          ...
          <div className="bg-light-base dark:bg-dark-card border border-light-border dark:border-dark-border rounded-xl p-5 sm:p-6">
            <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
              ...
              <div className="flex gap-1">
                {timeRanges.map(r => (
                  <button key={r} onClick={() => setActiveRange(r)} className={`px-2 py-1 text-[11px] font-semibold transition-colors ${activeRange === r ? 'bg-green-primary text-white' : 'text-slate-500 dark:text-slate-400 hover:text-dark-base dark:hover:text-white'}`}>{r}</button>
                ))}
              </div>
            </div>
        ...
        <div className="bg-light-base dark:bg-dark-card border border-light-border dark:border-dark-border rounded-xl">
          <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-light-border dark:border-dark-border">
            ...
            <Link href="/dashboard/transactions" className="text-xs font-semibold text-green-primary hover:text-green-dim transition-colors">View all</Link>
          </div>

          {transactions.length === 0 ? (
            <div className="py-10 text-center">
              <p className="text-sm text-slate-500 dark:text-slate-400">No transactions yet.</p>
              <Link href="/dashboard/deposit" className="inline-flex items-center gap-1.5 mt-3 text-sm font-semibold text-green-primary hover:text-green-dim transition-colors">
                  ...
                  <tbody className="divide-y divide-light-border dark:divide-dark-border">
                    {transactions.map(tx => (
                      <tr key={tx.id} className="hover:bg-light-surface dark:hover:bg-dark-surface transition-colors">
                        ...
                        <td className={`px-6 py-4 text-sm font-semibold ${tx.type === 'Withdrawal' ? 'text-green-primary' : 'text-dark-base dark:text-white'}`}>
                ...
                {transactions.map(tx => (
                  <div key={tx.id} className="px-5 py-4 flex items-center justify-between gap-4">
                    ...
                    <div className="text-right flex-shrink-0">
                      <p className={`text-sm font-semibold ${tx.type === 'Withdrawal' ? 'text-green-primary' : 'text-dark-base dark:text-white'}`}>
                        {tx.type === 'Withdrawal' ? '-' : '+'}${tx.amount.toLocaleString()}
                      </p>
