        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 text-center">
            <div className="w-16 h-16 rounded-full bg-white dark:bg-dark-card shadow-sm flex items-center justify-center mb-4">
              <MessageCircle size={28} className="text-green-primary" />
            </div>
                      ...
                      <div key={msg.id} className={`flex items-end gap-2.5 ${isUser ? 'justify-end' : 'justify-start'}`}>
                        ...
                        {isAdmin && (
                          <div className={`flex-shrink-0 mb-0.5 ${showAvatar ? 'visible' : 'invisible'}`}>
                            <div className="w-8 h-8 rounded-full bg-green-primary flex items-center justify-center shadow-sm">
                        ...
                        <div className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} max-w-[72%]`}>
                          ...
                          {showAvatar && (
                            <p className="text-[10px] font-bold text-green-primary uppercase tracking-wider mb-1 ml-1">
                              Support
                            </p>
                          )}
                          ...
                          <div className={`relative px-4 py-3 shadow-sm ${
                            isUser
                              ? 'bg-green-primary text-white rounded-3xl rounded-br-md'
      ...
      <div className="bg-light-base dark:bg-dark-base border-t border-light-border dark:border-dark-border px-4 py-3">
        <form onSubmit={handleSend} className="max-w-2xl mx-auto flex items-center gap-2">
          <input
            ...
            className="flex-1 px-4 py-3 rounded-full border border-light-border dark:border-dark-border bg-light-surface dark:bg-dark-card text-dark-base dark:text-white text-sm focus:outline-none focus:border-green-primary transition-colors placeholder:text-slate-400"
          />
          <button
            type="submit"
            disabled={!input.trim() || sending}
            className="w-11 h-11 flex items-center justify-center bg-green-primary hover:bg-green-dim disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-full transition-colors flex-shrink-0 shadow-sm"
          >
