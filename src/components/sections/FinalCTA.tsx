  ...
  return (
    <section className="bg-light-base dark:bg-dark-base py-24 lg:py-32 relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[400px] bg-green-primary/6 dark:bg-green-primary/8 blur-3xl" />
      </div>
      ...
      <div className="relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        >
          {/* Label */}
          <span className="inline-block text-xs font-semibold tracking-widest text-green-primary uppercase mb-6">
            Start Today
          </span>
          ...
          {!submitted ? (
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto mb-6">
              <input
                ...
                required
                className="flex-1 px-4 py-3.5 border border-light-border dark:border-dark-border bg-light-surface dark:bg-dark-card text-dark-base dark:text-light-base placeholder:text-slate-500 dark:placeholder:text-slate-400 text-sm focus:outline-none focus:border-green-primary transition-all duration-200"
              />
              <motion.button
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.97 }}
                type="submit"
                className="flex items-center justify-center gap-2 px-6 py-3.5 bg-green-primary text-white font-bold text-sm cursor-pointer hover:bg-green-dim transition-colors duration-200 whitespace-nowrap"
              >
                Get Started Free
                <ArrowRight size={16} />
              </motion.button>
