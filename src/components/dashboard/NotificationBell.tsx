  ...
  return (
    <div className="relative" ref={wrapperRef}>
      ...
      <button
        ...
        className="w-8 h-8 flex items-center justify-center border border-light-border dark:border-dark-border text-slate-500 dark:text-slate-400 hover:border-green-primary hover:text-green-primary transition-colors relative"
        aria-label="Notifications"
      >
        <Bell size={15} />
        {unread > 0 && (
          <span className="absolute -top-1.5 -right-1.5 min-w-[18px] h-[18px] px-0.5 bg-green-primary text-white text-[9px] font-bold flex items-center justify-center rounded-full">
            {unread > 9 ? '9+' : unread}
          </span>
        )}
      </button>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-light-border dark:border-dark-border">
            ...
            <div className="flex items-center gap-3">
              {unread > 0 && (
                <button
                  onClick={markAllRead}
                  className="text-xs text-slate-500 dark:text-slate-400 hover:text-green-primary transition-colors font-medium"
                >
                  Mark all read
                </button>
              )}
          ...
          <div className="max-h-[340px] overflow-y-auto divide-y divide-light-border dark:divide-dark-border">
            {notifications.length === 0 ? (
              ...
              notifications.map(n => (
                <div
                  key={n.id}
                  className={`px-4 py-3 flex items-start gap-3 transition-colors ${!n.read ? 'bg-green-primary/[0.04] dark:bg-green-primary/[0.06]' : ''}`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {!n.read && (
                        <span className="w-1.5 h-1.5 rounded-full bg-green-primary flex-shrink-0 mt-1" />
                      )}
                    </div>
